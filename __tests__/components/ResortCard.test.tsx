...
it('renders resort name and state', () => {
  const { getByText } = render(<ResortCard resort={base} />);
...
it('renders IKON pass badge for ikon resorts', () => {
  ...
  const { getByText } = render(<ResortCard resort={ikon} />);
...
it('renders "Snow data unavailable" when snow is null', () => {
  ...
  const { getByText } = render(<ResortCard resort={noSnow} />);
...
it('omits lift counts when lifts is null', () => {
  ...
  const { queryByText } = render(<ResortCard resort={noLifts} />);
