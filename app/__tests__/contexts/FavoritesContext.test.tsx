...
it('starts empty and reports loaded once storage has been read', async () => {
  ...
  await waitFor(() => expect(getByTestId('loaded').props.children).toBe('true'), { timeout: 4000 });
...
it('restores favorites saved on a previous visit', async () => {
  ...
  const { getByTestId } = renderProbe();
  await waitFor(() => expect(getByTestId('ids').props.children).toBe('vail,mammoth'), { timeout: 4000 });
});
...
it('adds a favorite and persists it to the device', async () => {
  ...
  await waitFor(() => expect(getByTestId('ids').props.children).toBe('vail'), { timeout: 4000 });
...
it('removes a favorite that is already saved', async () => {
  ...
  await waitFor(() => expect(getByTestId('ids').props.children).toBe('vail,mammoth'), { timeout: 4000 });
  fireEvent.press(getByTestId('toggle-vail'));
  await waitFor(() => expect(getByTestId('ids').props.children).toBe('mammoth'), { timeout: 4000 });
