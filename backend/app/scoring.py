...
Formula:
  snow_component = min(new_24h_in / 12.0, 1.0)  × 0.7
  lift_component = lifts_open / lifts_total       × 0.3
...
def compute_best_score(
    ...
    if all(v is None for v in [new_24h_in, lifts_open]):
        return None

    snow = min((new_24h_in if new_24h_in is not None else 0.0) / 12.0, 1.0) * 0.7
    lift = ((lifts_open if lifts_open is not None else 0) / lifts_total) * 0.3 if lifts_total else 0.0

    return round(snow + lift, 4)
